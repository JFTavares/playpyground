...
def main():
    ...
    while popl_fitness < 1 and generations < GENERATION_LIMIT:
        ...
        popl_fitness = fitness(parents, GOAL)
        print("Geração {} fitness = {:.4f}".format(generations,
                                                      popl_fitness))
        ave_wt.append(int(statistics.mean(parents)))
        generations += 1

    print("peso médio por geração = {}".format(ave_wt))
    print("\nnumero de gerações = {}".format(generations))
    print("numero de anos = {}".format(int(generations / LITTERS_PER_YEAR)))
